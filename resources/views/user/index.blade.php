...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2 align-items-center flex-wrap">
            ...
            <!-- Form Filter & Download User -->
            <form action="{{ route('user.download') }}" method="GET" class="d-flex gap-2 align-items-center mb-0">
                <select name="jurusan" class="form-select form-select-sm" style="width: 160px;">
                    ...
                    <!-- Silakan tambah atau sesuaikan singkatan jurusan lainnya di sini -->
                </select>

                <button type="submit" class="btn btn-info text-nowrap">
                    <i class="bx bx-export"></i> Download
                </button>
            </form>

            <a href="{{ route('siswa.template') }}" class="btn btn-success text-nowrap">
                <i class="bx bx-download"></i> Template CSV
            </a>

            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modal-tambah-user">
                        ...
                        @forelse ($users as $u)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-warning btn-sm btn-edit_user m-2" data-id="{{ $u->id }}"
                                        data-username="{{ $u->username }}" data-role="{{ $u->role }}"
                                        data-bs-toggle="modal" data-bs-target="#modal-edit" title="Edit">
                                        <i class="bx bx-edit"></i>
                                    </button>

                                    <button class="btn btn-danger btn-sm btn-icon m-2"
                                        onclick="confirmDelete({{ $u->id }})" title="Hapus">
