...
@section('content')
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body">
                    ...
                    <form id="formSelesai" method="POST" action="{{ route('ujianstart.latihanSelesai', $ujian->id) }}">
                        @csrf

                        <button id="btnSelesai" type="submit" class="btn btn-warning w-100 fw-bold">
                            Selesaikan Latihan
                        </button>
                    </form>
    ...
    <script>
        ...
        $(document).on('change', '.jawaban-radio', function() {
            ...
            // simpan jawaban terbaru
            jawabanUser[soalId] = jawaban;

            // reset semua radio visual
            $('.jawaban-radio').prop('checked', false);
            $('.opsi').removeClass('active');

            // centang hanya pilihan terbaru
            $(this).prop('checked', true);
            $(this).closest('.opsi').addClass('active');

            updateNavigasi();

            $.post("{{ route('ujian.latihan.save', $ujian->id) }}", {
                _token: "{{ csrf_token() }}",
                soal_id: soalId,
                jawaban: jawaban
            });
        ...
        $(document).on('click', '#btnSelesai', function(e) {
            ...
            Swal.fire({
                title: 'Yakin ingin menyelesaikan latihan?',
                icon: 'warning',
